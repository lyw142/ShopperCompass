...
def recommend_products(request):
    if request.method == 'POST':
        ...
        if min_price and max_price:
            ...
            for recommendation in recommendations_kmeans:
                if min_price <= recommendation['Price'] <= max_price:
                    filtered_recommendations.append(recommendation)
            recommendations_kmeans = filtered_recommendations

        # Sort recommendations by price in ascending order
        recommendations_kmeans = sorted(recommendations_kmeans, key=lambda x: x['Price'])
